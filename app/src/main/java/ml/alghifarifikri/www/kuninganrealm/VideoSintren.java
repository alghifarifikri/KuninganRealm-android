...
public class VideoSintren extends YouTubeBaseActivity implements YouTubePlayer.OnInitializedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_videosintren);

        youTubePlayerView = (YouTubePlayerView) findViewById(R.id.youtube_player13);

        //set the api key here
        youTubePlayerView.initialize(ConstantSintren.YOUTUBE_API_KEY,this);
    }

    @Override
    public void onInitializationSuccess(YouTubePlayer.Provider provider, YouTubePlayer youTubePlayer, boolean b) {
        if(!b){
            youTubePlayer.cueVideo(ConstantSintren.YOUTUBE_ID);
